...
void ts3plugin_registerPluginID(const char* id) {
    ...
    if (pluginID != 0) _strcpy(pluginID, sz, id);  /* The id buffer will invalidate after exiting this function */
...
void ts3plugin_infoData(uint64 serverConnectionHandlerID, uint64 id, enum PluginItemType type, char** data) {
    ...
    *data = (char*)malloc(INFODATA_BUFSIZE * sizeof(char));  /* Must be allocated in the plugin! */
    ...
    // You don't need to free this memory as its statically allocted as an array at the start of the program.
    // You only need to free memory that's dynamically allocated.
...
void addOrUpdateChannel(uint64 serverConnectionHandlerID, uint64 channelID)
{
    char *s;
    unsigned int err;

    // Get the channel name for the new channel
    err = ts3Functions.getChannelVariableAsString(serverConnectionHandlerID, channelID, CHANNEL_NAME, &s);

    // Assuming that the channel name came back OK...
    if (err == ERROR_ok)
    {
        // Get rid of the old channel record we have
        ts3channels.deleteChannel(channelID);

        // Try to add it to our list of channels (this will only happen if there's a valid frequency in the name)
        ts3channels.addOrUpdateChannel(string(s), channelID);
    }
...
void ts3plugin_onConnectStatusChangeEvent(uint64 serverConnectionHandlerID, int newStatus, unsigned int errorNumber) {
    ...
    if (newStatus == STATUS_DISCONNECTED ) {  /* connection established and we have client and channels available */
        ts3channels.deleteAllChannels();
    }
}

// New Channel Event - invoked once for each and every channel on connection
void ts3plugin_onNewChannelEvent(uint64 serverConnectionHandlerID, uint64 channelID, uint64 channelParentID) {

    addOrUpdateChannel(serverConnectionHandlerID, channelID);
...
void ts3plugin_onDelChannelEvent(uint64 serverConnectionHanderID, uint64 channelID, anyID invokerID, const char* invokerName, const char* invokerUniqueIdentifier)
{
    ts3channels.deleteChannel(channelID);
}
...
void ts3plugin_onTalkStatusChangeEvent(uint64 serverConnectionHandlerID, int status, int isReceivedWhisper, anyID clientID) {
    ...
    channelId = ts3channels.getChannelID(GetCom1());
   
    // If we've got a good channel name, then...
    if ( channelId != TS3Channels::CHANNEL_ID_NOT_FOUND)
    {
        // Get my client ID from the server
        ts3Functions.getClientID(serverConnectionHandlerID, &myId);

        // if it's me who's speaking...
        if (myId == clientID)
        {
            // Get which channel I'm in.
            uint64 uiMyChannel = 0;
            ts3Functions.getChannelOfClient(serverConnectionHandlerID, myId, &uiMyChannel);

            // If I'm not in the channel that belongs to the frequency I'm tuned to...
